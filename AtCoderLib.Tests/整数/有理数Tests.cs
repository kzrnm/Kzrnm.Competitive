...
{
    public class 有理数Tests
    {
        static IEnumerable<Fraction> RandomFractions(Xorshift rnd)
            => Enumerable.Repeat(rnd, 1000).Select(rnd => new Fraction(rnd.NextLong(), rnd.NextLong()));
        ...
        {
            foreach (var num in Enumerable.Repeat(new Xorshift(150), 1000).Select(rnd => rnd.NextLong()))
            {
                Fraction f = num;
        ...
        {
            foreach (var f in RandomFractions(new Xorshift(3153)))
            {
                var f2 = f;
                (f == f2).Should().BeTrue();
                (f != f2).Should().BeFalse();
        ...
        {
            foreach (var f in RandomFractions(new Xorshift(13)))
            {
        ...
        {
            foreach (var f in RandomFractions(new Xorshift(48463)))
            {
