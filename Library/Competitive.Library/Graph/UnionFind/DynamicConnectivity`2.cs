...
namespace Kzrnm.Competitive
{
    ...
    /// <list type="bullet">
    /// <item>
    /// <description>sと連結している頂点をfold（総和等）した結果を返す</description>
    /// </item>
    /// <item>
    /// <description>sと連結している頂点の総積の取得</description>
    /// </item>
    /// </list>
    /// <para>を O(log^2 N) で求めることが出来るデータ構造です。</para>
    /// </summary>
    public class DynamicConnectivity<T, TOp> where TOp : struct, ISegtreeOperator<T>
    {
        internal class Node
        {
            public Node ch0;
            ...
            public Node parent;
            public int l, r, size;
            public T val = op.Identity, prod = op.Identity;
            internal byte flag;
            public bool Exact
            {
                [凾(256)]
        ...
        class EulerianTourTree
        {
            ...
            private Node Update(Node t)
            {
                ...
                if (t.l == t.r) t.prod = op.Operate(t.prod, t.val);
                if (t.ch1 != null) t.prod = op.Operate(t.prod, t.ch1.prod);
