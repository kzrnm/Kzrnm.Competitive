...
using static Avx2;
    ...
    {
        [凾(256|512)]
        private static void MulSimd(Span<VectorizedModInt<T>> s, Span<VectorizedModInt<T>> t, Span<VectorizedModInt<T>> u)
        {
            for (int i = 0; i < B * B8; i++)
            {
                var cmpS = CompareGreaterThan(s[i].Value.AsInt32(), VectorizedModInt<T>.M1.AsInt32()).AsUInt32();
                var cmpT = CompareGreaterThan(t[i].Value.AsInt32(), VectorizedModInt<T>.M1.AsInt32()).AsUInt32();
                var difS = And(cmpS, VectorizedModInt<T>.M1);
                var difT = And(cmpT, VectorizedModInt<T>.M1);
                s[i] = Subtract(s[i].Value, difS);
                t[i] = Subtract(t[i].Value, difT);
            }

            var m1v = VectorizedModInt<T>.M1.GetElement(0);
            var m2v = VectorizedModInt<T>.M2.GetElement(0);

            var zero = new VectorizedModInt<T>().Value;
            var th1 = new VectorizedModInt<T>(0, m1v, 0, m1v, 0, m1v, 0, m1v).Value.AsInt64();
            var th2 = new VectorizedModInt<T>(0, m2v, 0, m2v, 0, m2v, 0, m2v).Value.AsInt64();
...
prod1370 = Subtract(prod1370, dif1370.AsUInt64());
  ...
  u[(i + 0) * B8 + j + 0] = VectorizedModInt<T>.Reduce(prod0200.AsUInt32(), prod1300.AsUInt32());
  ...
  u[(i + 1) * B8 + j + 0] = VectorizedModInt<T>.Reduce(prod0210.AsUInt32(), prod1310.AsUInt32());
  ...
  u[(i + 2) * B8 + j + 0] = VectorizedModInt<T>.Reduce(prod0220.AsUInt32(), prod1320.AsUInt32());
  ...
  u[(i + 3) * B8 + j + 0] = VectorizedModInt<T>.Reduce(prod0230.AsUInt32(), prod1330.AsUInt32());
  ...
  u[(i + 4) * B8 + j + 0] = VectorizedModInt<T>.Reduce(prod0240.AsUInt32(), prod1340.AsUInt32());
  ...
  u[(i + 5) * B8 + j + 0] = VectorizedModInt<T>.Reduce(prod0250.AsUInt32(), prod1350.AsUInt32());
  ...
  u[(i + 6) * B8 + j + 0] = VectorizedModInt<T>.Reduce(prod0260.AsUInt32(), prod1360.AsUInt32());
  ...
  u[(i + 7) * B8 + j + 0] = VectorizedModInt<T>.Reduce(prod0270.AsUInt32(), prod1370.AsUInt32());
              }
            }
        }
    }
