...
namespace Kzrnm.Competitive
{
    public static partial class __MatrixProdStrassen_StaticMod
    {
        ...
        /// </remarks>
        public static ArrayMatrix<StaticModInt<T>> Strassen<T>(
            this ArrayMatrix<StaticModInt<T>> mat1,
            ArrayMatrix<StaticModInt<T>> mat2) where T : struct, IStaticMod
        {
        ...
        [凾(256)]
        static VectorizedModInt<T>[] ToVectorize<T>(ArrayMatrix<StaticModInt<T>> m, int size, int s8) where T : struct, IStaticMod
        {
            var v = m.Value;
            var w = m.Width;
            ...
            for (int i = m.Height - 1; i >= 0; i--)
            {
                ...
                MemoryMarshal.Cast<VectorizedModInt<T>, uint>(rt.AsSpan(i * s8)));
            }
            return rt;
        }

        [凾(256)]
        static ArrayMatrix<StaticModInt<T>> ToMatrix<T>(VectorizedModInt<T>[] c, int h, int w, int s8) where T : struct, IStaticMod
        {
            Debug.Assert(h * w <= c.Length * 8);
            var rt = new StaticModInt<T>[h * w];
            for (int i = 0; i < h; i++)
            {
                var src = MemoryMarshal.Cast<VectorizedModInt<T>, uint>(c.AsSpan(s8 * i));
