...
namespace Kzrnm.Competitive
{
    ...
    public static class ManhattanMST
    {
        ...
        public static (int Index1, int Index2)[] Solve((int, int)[] ps) => Solve<int, IntOperator>(ps);
        /// <summary>
        /// マンハッタン距離 |x2-x1| + |y2-y1| での最小全域木を構築する
        /// </summary>
        /// <param name="ps">座標の一覧</param>
        public static (int Index1, int Index2)[] Solve((long, long)[] ps) => Solve<long, LongOperator>(ps);
        /// <summary>
        /// マンハッタン距離 |x2-x1| + |y2-y1| での最小全域木を構築する
        /// </summary>
        /// <param name="ps">座標の一覧</param>
        public static (int Index1, int Index2)[] Solve<T, TOp>((T, T)[] ps)
            ...
            , ICompareOperator<T>
        {
            ...
            for (int ph = 0; ph < 4; ph++)
            {
                ...
                Array.Sort(xv);
                var fw = new FenwickTree<(T, int), MaxFenwickTreeOperator<T>>(ps.Length);
            ...
            Array.Sort(edges.Select(new Dist<T, TOp>(ps).Convert).ToArray(), edges, 0, edgesList.Count);
        ...
        struct Dist<T, TOp> : IComparer<(int Index1, int Index2)>
            where TOp : struct
            , IAdditionOperator<T>
            , ISubtractOperator<T>
            , IUnaryNumOperator<T>
...
*/
    ...
    struct MaxFenwickTreeOperator<T> : IAdditionOperator<(T, int)>, ISubtractOperator<(T, int)>
            where T : IComparable<T>
    {
        [凾(256)]
