...
public class PersistentStackDoublingTests
{
    ...
    public async Task Stack()
    {
        ...
        {
            ss[i] = ss[i - 1].Push(i);
            await ss[i].Should().BeStrictlyEquivalentTo(Enumerable.Range(1, i).Reverse());
        ...
        for (int i = ss.Length - 1; i >= 0; i--)
        {
            ...
            {
                var other1 = ss[i].Pop();
                await other1.Should().BeStrictlyEquivalentTo(Enumerable.Range(1, i - 1).Reverse());
                await other1.Count.Should().BeEqualTo(i - 1);
            }

            var add2 = ss[i].Push(i).Push(i);
            await add2.Count.Should().BeEqualTo(i + 2);
            await add2.Should().BeStrictlyEquivalentTo(Enumerable.Range(1, i).Append(i).Append(i).OrderBy(i => i).Reverse());
        }

        await ss[50].Push(-1).Push(-3).Should().BeStrictlyEquivalentTo(new[] { -3, -1 }.Concat(Enumerable.Range(1, 50).Reverse()));
    }
    [Test, MultipleAssertions]
