...
public class ZahyoCompressTests
{
    ...
    public async Task Compress()
    {
        ...
        using (Assert.Multiple())
        {
            ...
            await c.Original.Should().BeStrictlyEquivalentTo(['c', 'e', 'm', 'o', 'p', 'r', 's']);
            await c.Replace("compress").Should().BeStrictlyEquivalentTo([0, 3, 2, 4, 5, 1, 6, 6]);
        }
        ...
        using (Assert.Multiple())
        {
            ...
            await c.Original.Should().BeStrictlyEquivalentTo(['s', 'r', 'p', 'o', 'm', 'e', 'c']);
            await c.Replace("compress").Should().BeStrictlyEquivalentTo([6, 3, 4, 2, 1, 5, 0, 0]);
        }
        ...
        using (Assert.Multiple())
        {
            ...
            await c.Original.Should().BeStrictlyEquivalentTo(['c', 'e', 'i', 'k', 'm', 'o', 'p', 'r', 's']);
        }
    }
    ...
    {
        await ZahyoCompress.CompressedArray("compress".AsSpan()).Should().BeStrictlyEquivalentTo([0, 3, 2, 4, 5, 1, 6, 6]);
